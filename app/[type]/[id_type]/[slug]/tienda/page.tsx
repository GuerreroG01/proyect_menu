...
export default function StorePage(props: {
    ...
    const safeCategory = activeCategory || data?.categories?.[0]?.id || "";
    ...
    const itemsToShow = itemsBase.filter((item: ProductItem) => {
        if (gender === "todos") return true;

        const mapped =
            item.target === "masculino"
            ? "caballero"
            : item.target === "femenino"
            ? "dama"
            : "unisex";

        return mapped === gender || item.target === "unisex";
    });
        ...
        <div className="min-h-screen bg-[#F8FAFC] flex flex-col">
            ...
            <main className="p-5 max-w-2xl mx-auto w-full flex-1 mb-32">
                ...
                <AnimatePresence mode="wait">
                    <motion.div
                        key={activeCategory + searchQuery + gender}
                    ...
                    >
                        {itemsToShow.map((item: ProductItem, index: number) => (
                            <StoreItemCard
                                key={item.id}
                                index={index}
                                item={item}
