...
const BADGE_CONFIG: Record<
    string,
    { label: string; styles: string; icon: React.ComponentType<{ size?: number }> }
> = {
    menus: { label: "Menú", styles: "text-amber-700 bg-amber-50", icon: UtensilsCrossed },
    restaurant: { label: "Menú", styles: "text-amber-700 bg-amber-50", icon: UtensilsCrossed },
    store: { label: "Tienda", styles: "text-emerald-700 bg-emerald-50", icon: Shirt },
    bakery: { label: "Catálogo", styles: "text-indigo-700 bg-indigo-50", icon: Croissant },
};
            ...
            <main className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 w-full">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    {negocios.map((res: any) => {
                        // Si no encuentra el tipo en la lista, usa valores por defecto (Catálogo / LayoutGrid)
                        const badge = BADGE_CONFIG[res.type] || { 
                            label: "Catálogo", 
                            styles: "text-indigo-700 bg-indigo-50", 
                            icon: LayoutGrid 
                        };
                                ...
                                <div>
                                    <div className="flex items-start justify-between mb-6 relative z-10">
                                        ...
                                        <div className="flex flex-col items-end gap-2">
                                            <span className={`inline-flex items-center gap-1 text-[10px] font-extrabold uppercase tracking-wider px-2 py-0.5 rounded-md ${badge.styles}`}>
                                                <BadgeIcon size={10} />
                                                {badge.label}
                                            </span>
                                        </div>
